...
function onOpen() {
  try {
    SpreadsheetApp.getUi()
      ...
      .addItem('📊 Update Tab REKAP_PRODUKSI (Varian Anak & Dewasa)', 'setupRekapProduksi')
      .addSeparator()
...
/** Resolusi SKU dari MASTER_PRODUK atau fallback terprogram (mendukung SKU dengan warna & ukuran anak/dewasa) */
function resolveProduk_(sku, prodMap) {
  ...
  // Helper parsing detail SKU (termasuk varian anak AK-)
  function parseSkuDetails_(rest) {
    ...
    if (parts.length === 3) {
      color = parts[0];
      sz = (parts[1] === 'AK' || parts[1] === 'KIDS') ? 'Anak ' + parts[2] : parts[1] + '-' + parts[2];
    } else if (parts.length === 2) {
      if (parts[0] === 'AK' || parts[0] === 'KIDS') {
        sz = 'Anak ' + parts[1];
      } else {
        color = parts[0];
        sz = parts[1];
    ...
    return { colorLabel: colorLabel, sz: sz };
  }

  // Fallback terprogram jika tidak ditemukan di tab MASTER_PRODUK
  if (s.indexOf('BD-KP') === 0) {
    const p = parseSkuDetails_(s.replace(/^BD-KP-/, ''));
    return { sku: s, nama: 'Paket Bundling (Kaos Pendek' + p.colorLabel + ' + Tumbler 750ml + Korek + Keychain + Goodie Bag)', ukuran: p.sz, harga: 210000 };
  }
  if (s.indexOf('BD-KJ') === 0) {
    const p = parseSkuDetails_(s.replace(/^BD-KJ-/, ''));
    return { sku: s, nama: 'Paket Bundling (Kaos Panjang' + p.colorLabel + ' + Tumbler 750ml + Korek + Keychain + Goodie Bag)', ukuran: p.sz, harga: 210000 };
  }
  if (s.indexOf('KP-') === 0) {
    const p = parseSkuDetails_(s.replace(/^KP-/, ''));
    return { sku: s, nama: 'Kaos Lengan Pendek' + p.colorLabel, ukuran: p.sz, harga: 120000 };
  }
  if (s.indexOf('KJ-') === 0) {
  ...
  data.forEach(function(d, i) {
    ...
    if (sisaJam <= 0) {
      ...
      kirimWA_(d[4],
        ...
        P_('NAMA_BANK', 'BCA') + ' ' + P_('NOMOR_REKENING', '7710399993') + '\na.n. ' + P_('ATAS_NAMA', 'Abidatul Faricha Ch') + '\n\n' +
...
/**
 * Memperbarui struktur & rumus tab REKAP_PRODUKSI di Google Sheets (Termasuk Varian Dewasa & Anak-Anak)
 */
function setupRekapProduksi() {
  ...
  shRekap.getRange('A1:I1').setValues([['MODEL DEWASA (LUNAS)', 'S', 'M', 'L', 'XL', 'XXL', '2XL', '3XL', 'TOTAL']]);
  ...
  // 2. Matriks Kaos Anak-Anak (Lunas)
  shRekap.getRange('A5:G5').setValues([['MODEL ANAK-ANAK (LUNAS)', 'Anak XS', 'Anak S', 'Anak M', 'Anak L', 'Anak XL', 'TOTAL']]);
  shRekap.getRange('A6').setValue('Kaos Lengan Panjang');
  const colsKids = ['B', 'C', 'D', 'E', 'F'];
  ...
  shRekap.getRange('A17:G17').setValues([['PROYEKSI ANAK-ANAK (TERMASUK UNPAID)', 'Anak XS', 'Anak S', 'Anak M', 'Anak L', 'Anak XL', 'TOTAL']]);
  ...
  shRekap.getRange('A20:B20').setValues([['ITEM NON-KAOS (PROYEKSI)', 'TOTAL PROYEKSI']]);
  ...
  // Formatting header
  try {
    shRekap.getRange('A1:I1').setBackground('#1e293b').setFontColor('#ffffff').setFontWeight('bold');
    shRekap.getRange('A5:G5').setBackground('#dc2626').setFontColor('#ffffff').setFontWeight('bold');
    shRekap.getRange('A8:B8').setBackground('#0284c7').setFontColor('#ffffff').setFontWeight('bold');
    shRekap.getRange('A13:I13').setBackground('#475569').setFontColor('#ffffff').setFontWeight('bold');
    shRekap.getRange('A17:G17').setBackground('#991b1b').setFontColor('#ffffff').setFontWeight('bold');
